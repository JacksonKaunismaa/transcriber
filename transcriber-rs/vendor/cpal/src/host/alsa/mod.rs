...
impl Device {
    fn build_stream_inner(
        ...
        let stream_inner = StreamInner {
            ...
            can_pause,
            creation_instant,
            pause_signal: (Mutex::new(false), Condvar::new()),
        };
...
struct StreamInner {
    ...
    // Patch (cpal#785 workaround): gates the worker thread when paused so it
    // sleeps on the condvar instead of busy-spinning in alsa::poll() — which
    // happens whenever snd_pcm_pause() is called against the ALSA pulse PCM
    // plugin (i.e. PipeWire-Pulse).
    pause_signal: (Mutex<bool>, Condvar),
}
...
/// Block while the stream's pause flag is set. Returns immediately if not
/// paused. Used by both worker loops to honour pause without spinning in
/// alsa::poll() (see cpal#785 and the pause_signal field on StreamInner).
fn wait_while_paused(stream: &StreamInner) {
    let (lock, cvar) = &stream.pause_signal;
...
fn input_stream_worker(
    ...
    loop {
        // cpal#785 workaround: block here when paused so the alsa-pulse PCM
        // can't drive the poll loop into a 100% CPU spin.
        wait_while_paused(stream);
        let flow =
...
impl Drop for Stream {
    fn drop(&mut self) {
        // Wake any cvar-blocked worker first so it can observe the shutdown
        // signal that follows. Otherwise the join below would deadlock if the
        // stream was paused at drop time.
        let (lock, cvar) = &self.inner.pause_signal;
...
impl StreamTrait for Stream {
    ...
    fn pause(&self) -> Result<(), PauseStreamError> {
        // Set the gate before asking ALSA to pause, so the worker is already
        // committed to the cvar before snd_pcm_pause() perturbs poll state.
        let (lock, _) = &self.inner.pause_signal;
